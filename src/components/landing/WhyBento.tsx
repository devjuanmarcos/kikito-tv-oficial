  ...
  return (
    <section className="kk-section" id="why" aria-labelledby="why-title">
      <style>{`
        ...
        .ks-bento { display: grid; grid-template-columns: repeat(12, minmax(0, 1fr)); gap: 8px; }
        ...
        .bt { grid-column: span var(--bt-span, 4); display: flex; flex-direction: column; gap: 1rem; padding: clamp(1.25rem, 2.4vw, 2rem); background: color-mix(in oklch, var(--ks-lacquer-raised) 55%, transparent); border: 1px solid var(--ks-rule); overflow: hidden; min-height: 220px; transition: border-color .25s, background .25s; }
        .bt:hover { border-color: color-mix(in oklch, var(--kk-accent) 45%, var(--ks-rule)); }
        @media (max-width: 980px) { .bt { grid-column: span 1 !important; } }
        .bt--8 { --bt-span: 8; } .bt--6 { --bt-span: 6; } .bt--4 { --bt-span: 4; }
        .bt-title { font-size: 1.0625rem; font-weight: 600; color: var(--ks-text); letter-spacing: -0.01em; }
        .bt-body { font-size: 0.875rem; line-height: 1.6; color: var(--ks-text-muted); max-width: 46ch; }
        ...
        .bt-play { width: 44px; height: 44px; border-radius: 50%; display: grid; place-items: center; color: var(--ks-primary-fg); background: var(--ks-primary); padding-left: 2px; }
        ...
        .bt-grid span { aspect-ratio: 1; border-radius: 4px; border: 1px solid var(--ks-rule); background: color-mix(in oklch, var(--ks-lacquer-raised) 70%, transparent); animation: bt-pop 2.6s var(--kk-ease-out) infinite; }
        @keyframes bt-pop { 0%, 70%, 100% { background: color-mix(in oklch, var(--ks-lacquer-raised) 70%, transparent); } 35% { background: color-mix(in oklch, var(--kk-accent) 35%, transparent); } }

        /* news */
        .bt-news { display: flex; flex-direction: column; gap: 8px; }
        .bt-news-row { display: flex; align-items: center; gap: 10px; padding: 9px 12px; border: 1px solid var(--ks-rule); border-radius: 8px; font-size: 0.8125rem; color: var(--ks-text-muted); background: color-mix(in oklch, var(--ks-lacquer-raised) 50%, transparent); }
