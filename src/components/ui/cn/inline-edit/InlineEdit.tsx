...
const PenIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true" className="w-3 h-3 opacity-40">
...
export function InlineEdit({
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    ...
    if (e.key === 'Enter' && multiline && (e.metaKey || e.ctrlKey)) { e.preventDefault(); confirm() }
  }
  ...
  return (
    ...
    >
      <span className={cn('min-w-[4ch]', !value && 'text-faint italic')}>
        {value || placeholder}
      </span>
      {!disabled && <PenIcon />}
    </button>
