...
const SERIES_COLORS = [
  'var(--ks-patina)', 'var(--ks-kinpaku)', 'var(--ks-info)',
  'var(--ks-success)', 'var(--ks-warning)', 'var(--ks-danger)',
]

interface TooltipState { x: number; y: number; index: number }
...
export function AreaChart({
  ...
  const stackedData = stacked
    ? numericData.map(row =>
        row.reduce<number[]>((acc, val, i) => { acc.push((acc[i - 1] ?? 0) + val); return acc }, [])
      )
    : numericData
  ...
  const makeAreaPath = (si: number) => {
    ...
    const bottom = stacked && si > 0
      ? data.map((_, i) => yOf(stackedData[i]?.[si - 1] ?? 0))
      : data.map(() => yOf(0))
      ...
      >
        ...
        {showGrid && yTickVals.map((v, i) => (
          <line key={i} x1={PAD.left} x2={PAD.left + innerW}
            y1={yOf(v)} y2={yOf(v)} stroke="var(--ks-rule)" strokeWidth={1} />
        ))}

        {yTickVals.map((v, i) => (
          <text key={i} x={PAD.left - 6} y={yOf(v) + 4} textAnchor="end"
        ...
        {data.map((d, i) => (
          <text key={i} x={xOf(i)} y={H - PAD.bottom + 16} textAnchor="middle"
            fontSize={10} fill="var(--ks-text-faint)">
            {String(d.label)}
          </text>
        ))}
        ...
        {[...series].reverse().map((s, ri) => {
          ...
          return (
            <path key={s.key + '-area'} d={makeAreaPath(i)}
        ...
        {series.map((s, i) => (
          <path key={s.key + '-line'} d={makeLinePath(i)}
        ...
        {showDots && series.map((s, si) =>
          data.map((_, di) => {
            ...
            return (
              <circle key={`${s.key}-${di}`} cx={xOf(di)} cy={yOf(v)} r={3}
                fill={getColor(si)} stroke="var(--ks-lacquer)" strokeWidth={1.5} />
            )
          })
        )}

        {tooltip && (
          <line x1={xOf(tooltip.index)} x2={xOf(tooltip.index)}
            y1={PAD.top} y2={PAD.top + innerH}
            stroke="var(--ks-rule)" strokeWidth={1} strokeDasharray="4 3" />
        )}
      </svg>
