...
export function CountdownTimer({
  ...
  return (
    <div
      className={cn("inline-flex items-center gap-(--spacing-2xs) tabular-nums", className)}
      style={style}
      data-done={time.done}
      role="timer"
      aria-live="off"
      ...
      {parts.map((p, i) => (
        <Fragment key={p.label}>
          <div className="flex flex-col items-center min-w-[48px]">
            <span
              className={cn(
                "text-heading-04 font-extrabold leading-none tabular-nums",
                time.done ? "text-danger" : "text-foreground"
              )}
            >
              {pad(p.value)}
            </span>
            {showLabels && (
              <span className="text-body-caption font-semibold text-muted uppercase tracking-[0.06em] opacity-50 mt-(--spacing-2xs)">
                {p.label}
              </span>
            )}
          </div>
          {i < parts.length - 1 && (
            <span
              className="text-heading-04 font-extrabold text-patina self-start pt-(--spacing-3xs) select-none"
              aria-hidden="true"
            >
              {separator}
            </span>
