...
export function Select({
  ...
  return (
    <div ref={wrapRef} className={cn('relative flex flex-col gap-[0.375rem]', className)} style={style}>
      ...
      >
        {iconLeft && (
          <span className="shrink-0 pl-3 text-faint [&>svg]:w-4 [&>svg]:h-4">{iconLeft}</span>
        )}
        ...
        <div className="absolute top-[calc(100%+4px)] left-0 right-0 z-[300] bg-lacquer border border-rule rounded-[--radius-sm] shadow-[0_8px_24px_-8px_oklch(0%_0_0/0.45)] overflow-hidden">
          ...
          <div className="max-h-[240px] overflow-y-auto py-1">
            {filtered.length === 0 ? (
              ...
              isGroup(item) ? (
                <div key={gi}>
                  <p className="px-3 pt-3 pb-1 text-[0.625rem] font-bold uppercase tracking-[0.1em] text-faint">{item.label}</p>
                  {item.options.map(opt => (
                    <OptionRow key={opt.value} opt={opt} current={current} onSelect={select} />
                  ))}
                </div>
              ) : (
                <OptionRow key={(item as SelectOption).value} opt={item as SelectOption} current={current} onSelect={select} />
              )
            )}
          </div>
        </div>
...
function OptionRow({ opt, current, onSelect }: { opt: SelectOption; current: string; onSelect: (o: SelectOption) => void }) {
