...
export function Command({
  ...
  function onKeyDown(e: React.KeyboardEvent) {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); const item = flat[activeIdx]; if (item) select(item) }
  }
  ...
  return createPortal(
    <div
      ...
      onMouseDown={e => { if (e.target === e.currentTarget) show(false) }}
    >
      <div
        ...
        style={{ animationName: 'ks-cmd-in', animationDuration: '140ms', animationTimingFunction: 'cubic-bezier(0.16,1,0.3,1)', animationFillMode: 'both' }}
        role="dialog"
        aria-modal="true"
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain p-[0.375rem]" ref={listRef} role="listbox">
          {filteredGroups.length === 0 ? (
            ...
            <div key={gi} className="[&+div]:border-t [&+div]:border-rule [&+div]:mt-1 [&+div]:pt-1">
              {group.heading && (
                <div className="text-[0.625rem] font-bold tracking-[0.1em] uppercase text-faint px-[0.625rem] py-[0.375rem] pb-1">{group.heading}</div>
              )}
