  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <div>
        <div
          ref={topScrollRef}
          className="relative w-full max-w-full overflow-x-auto overflow-y-hidden"
          style={{ height: 8, marginBottom: 4 }}
        >
          <div style={{ width: tableWidth, height: 1 }} />
        </div>
        <div ref={bottomScrollRef} className="relative w-full max-w-full overflow-x-auto">
          <table
            ref={(node) => {
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn("border-b border-border transition-colors hover:bg-card data-[state=selected]:bg-muted-surface", className)}
      {...props}
    />
  )
);
