...
**Achado original (sessão anterior), REVISITADO E REBAIXADO nesta sessão — não é bug de componente.** A caracterização original era: o modo `confirm="doubleclick"` do `ConfirmImpl` "não atualiza visualmente em resposta a cliques reais/confiáveis", só clique programático funcionava. Investigação nova, com reprodução ao vivo no Browser pane (não só Playwright):

- **Componente confirmado CORRETO pra usuários reais.** Reproduzido com `MutationObserver` anexado direto no botão: um clique real via `computer` tool (clique OS-level, `isTrusted:true`) na página `feedback/confirm-button` atualiza o texto pra "Click again to confirm" em ~1ms e reverte sozinho ~2000ms depois (`resetDelay` default), exatamente como projetado — tanto nessa página quanto em `inputs/button`. `el.click()` programático também funciona perfeitamente nas duas.
- **O que de fato acontece**: especificamente via Playwright, especificamente na página `feedback/confirm-button`, toda forma de clique (`.click()`, `.click({force:true})`, `page.mouse.click()` em coordenadas medidas ao vivo, `.dispatchEvent('click')`, `Enter` com foco, e até `el.click()` chamado via `page.evaluate`) falha em atualizar o texto — mesmo com 2s de espera extra pós-`networkidle` e 3 cliques seguidos. Confirmado via inspeção do fiber do React que `onClick` está corretamente anexado (`typeof props.onClick === 'function'`) — não é falta de hidratação.
- **A MESMA demo (`ConfirmButtonDemo`) funciona normalmente via Playwright na página `inputs/button`** (`e2e/cn/inputs/button.spec.ts`, teste novo "confirm='doubleclick' real: clique real atualiza o label antes do auto-reset de 2s", 8/8 passou chromium-desktop + mobile-chrome) — ou seja, o mesmo código de componente, chamado com os mesmos props, funciona via Playwright numa página e falha na outra.
- **Hipóteses eliminadas nesta rodada** (além das já eliminadas antes — `.next` corrompido, HMR, StrictMode, `active:scale`, handlers mousedown/up): anúncios/reCAPTCHA da página (`feedback/confirm-button` carrega 3 iframes de ads via `adsbygoogle.js`, injetado incondicionalmente em `layout.tsx` pro site inteiro — bloqueados via `page.route(...).abort()`, bug persiste **idêntico** com zero iframes presentes, então não é interferência de anti-fraude de anúncio/bot-detection); foco/visibilidade da página (`document.hasFocus()` `true`, `visibilityState` `"visible"` durante o teste); hydration incompleta (2s de espera extra antes do primeiro clique não muda nada, nem 3 cliques consecutivos).
- **Causa raiz ainda não isolada** — resiste a uma segunda rodada de investigação tão extensa quanto a primeira. É uma anomalia confirmada do ambiente Playwright especificamente nesta rota, não um defeito do componente.

Teste mantido como `test.fail(...)` em `confirm-button.spec.ts` (com comentário atualizado explicando a nova caracterização) — documenta a anomalia sem mascará-la, mas **rebaixado de "achado grave, prioridade alta" pra curiosidade de infraestrutura de teste, não bloqueante**: usuários reais não são afetados (confirmado por múltiplos métodos de interação genuína), e o comportamento correto já tem cobertura Playwright verde via a página `inputs/button`.
...
### `layout/navigation-menu` — concluído

Centésimo vigésimo nono standalone. Nunca tinha sido tocado — sem nenhuma alegação de absorção.

- **Achado grave real**: itens com `href` renderizavam `<button onClick>` em vez de `<a href>` — sem suporte a ctrl/cmd/shift-click (abrir em nova aba), clique do meio, "copiar link" do menu de contexto, ou navegação nativa quando `onNavigate` não é passado; pra leitor de tela, itens de navegação eram anunciados como "botão", não "link". Corrigido: itens-folha (sem `children`) com `href` agora renderizam `<a>` de verdade, com um handler (`handleLinkClick`) que só faz `preventDefault`+chama `onNavigate` em clique simples sem modificador — clique com Ctrl/Cmd/Shift/botão do meio passa direto pro navegador. Itens COM `children` continuam `<button>` (função é abrir o submenu, não navegar — comportamento original preservado).
- **Gate 5, achado real**: dropdown fechava clicando fora, mas não tinha `Escape` nenhum — único widget flutuante desta biblioteca sem esse fechamento (`Tooltip`/`ImageViewer`/`OnboardingTour`/`NotificationBell` já tinham). Adicionado listener de teclado.
- Gate 2, achado real, mesma categoria já corrigida dezenas de vezes: `bg-patina/15` (item ativo) reimplementava à mão o que `bg-patina-soft` já faz → substituído.
- Gate 4, achado real: badge (`bg-danger text-danger-fg rounded-full text-[0.6rem]`) reimplementava exatamente `Badge intent="danger" variant="solid" size="sm"` → substituído nos dois níveis (item de topo e item do submenu).
- Gate 1: `.types.ts` extraído (tipos estavam inline no `.tsx`); aspas simples → duplas.
- Gate 3: `gap-1.5`/`px-3`/`py-2`/`px-1.5`/`py-0.5`/`gap-4`/`px-4`/`gap-0.5`/`mt-1`/`ml-4`/`mt-0.5` (todos match exato, arquivo nunca tinha sido tokenizado) → tokens de spacing.
- **Achado real no registry**: `items` documentava um campo `id` que **não existe** no tipo real, e omitia `disabled`/`badge` (ambos reais) — mesma origem do achado: o showcase também tinha `id`/`description` cru nos dados de exemplo, nunca usados pelo componente (limpo de passagem). `style` (real) também ausente. Corrigidos + `registry:build`.
- Showcase: só tinha 1 variação horizontal sem `badge`/`disabled`/`onNavigate`/orientação vertical — nenhum dos achados corrigidos era exercitável. Reescrito com badge, item disabled, `onNavigate`, `activeHref` e uma segunda seção vertical.
- Gate 9: `e2e/cn/layout/navigation-menu.spec.ts` novo (6 testes: crash/console/item com href é `<a>` de verdade/dropdown fecha com Escape/item disabled tem `aria-disabled`/badge via `Badge` CN) — 11/11 reais passando (1 flake isolado de CSP `frame-ancestors`/`google.com`, mesma categoria já documentada no `ThemeSelector`, não reproduziu ao re-rodar 2x isolado).

### `display/ping` — concluído

Centésimo trigésimo standalone. `badge→tag/status-badge/ping` já confirmado como absorção falsa, mas `Ping` em si nunca tinha passado pelos 9 gates.

- **Gate 2, achado real**: `intent="neutral"` era o único dos 6 intents que não seguia o próprio padrão do arquivo (dot = cor sólida do intent, ring = mesma cor a 40%) — usava `bg-foreground/50`/`bg-foreground/20` (diluição ad-hoc) em vez do token `bg-neutral` que **já existe** no projeto (mesmo usado no `neutral/solid` do `Button`). Corrigido pra `bg-neutral`/`bg-neutral/40`, consistente com os outros 5 intents.
- **Gate 5, achado real, mesma categoria do `StatusBadge`**: o dot persistente (o indicador de status de verdade, não o anel de pulso decorativo) não tinha `role`/`aria-label` nenhum — comunicava status **só por cor**, invisível pra leitor de tela. Diferente do `StatusBadge` (vocabulário fixo tipo "Online"/"Busy"), `Ping` não tem um conjunto fixo de strings de status (`intent` é só uma cor semântica, não uma palavra de status) — corrigido com uma nova prop opcional `label` (sem exigir prop obrigatória nova pra quem já usa o componente); quando fornecida, o dot ganha `role="img"` + `aria-label`, e o dot interno vira `aria-hidden` pra não duplicar o anúncio.
- Gate 2, avaliado e mantido: `bg-{intent}/40` do anel de pulso (`animate-ping`) — aqui a opacidade É o mecanismo da animação (o anel começa translúcido e desvanece), categoria diferente da diluição de texto/fundo já banida; mesmo padrão já aceito pro `pulse` do `StatusBadge`.
- **Achado real no registry**: `label` (nova, real) e `style` (real, já existia) ausentes → adicionados + `registry:build`.
- Showcase: já cobria os 6 intents + 2 tamanhos com `children`, mas nenhuma instância com `label` → adicionada.
- Gate 9: `e2e/cn/display/ping.spec.ts` novo (4 testes: crash/console/`label` expõe `role="img"`/`neutral` usa `bg-neutral` sem diluição de foreground) — 8/8 chromium-desktop + mobile-chrome, direto na primeira rodada.

### `display/progress-steps` — concluído

Centésimo trigésimo primeiro standalone. `stepper→dot-stepper/progress-steps` já confirmado como absorção falsa, mas `ProgressSteps` em si nunca tinha passado pelos 9 gates. Componente já muito bem construído de origem (`aria-current="step"` já correto desde a implementação, todos os tamanhos abaixo do mínimo da escala já documentados com comentário).

- **Gate 5, achado real, mesmo padrão já estabelecido no `StepForm`**: tinha `aria-current="step"` mas não `role="list"`/`role="listitem"` na sequência de etapas — mesma combinação já usada no `StepForm` pro idêntico conceito (sequência de passos). Adicionado.
- Gate 2/3: nada a corrigir — `text-faint` no label de passos "upcoming" e na descrição avaliados e mantidos (indicação de estado ainda-não-alcançado e metadado secundário, respectivamente — não é o padrão de diluição já banido); os 3 tamanhos abaixo do mínimo da escala já tinham comentário de exceção desde a implementação original.
- **Achado real no registry**: descrição de `steps` omitia o campo `status?` (real); `style` (real) ausente. Corrigidos + `registry:build`.
- Showcase: já cobria horizontal, vertical e estado "tudo completo".
- Gate 9: `e2e/cn/display/progress-steps.spec.ts` novo (3 testes: crash/console/`role="list"`+`listitem`+`aria-current="step"` no passo certo) — 5 passed + 1 skip (pendência 0b mobile).

### `layout/scroll-spy` — concluído

Centésimo trigésimo segundo standalone. `scroll-spy→table-of-contents` já confirmado como absorção falsa, mas `ScrollSpy` em si nunca tinha passado pelos 9 gates. Registry já vinha 100% correto (comentário de não-absorção já documentado desde antes).

- **Gate 5, achado real**: item ativo usava `aria-current="true"` — tecnicamente válido, mas o WAI-ARIA tem um token específico pra esse caso exato: `aria-current="location"` ("elemento representando a localização atual dentro de um contexto"), mais preciso que o booleano genérico. Mesma precisão já aplicada a `aria-current="step"` no `StepForm`/`ProgressSteps` — corrigido pro token certo.
- **Gate 5, achado real**: clique num item chama `scrollIntoView({behavior:'smooth'})` — diferente de animação CSS (`@keyframes`), essa API do browser **não** respeita `prefers-reduced-motion` sozinha quando o valor vem direto do JS. Primeira ocorrência dessa categoria específica nesta auditoria (as anteriores eram todas CSS). Corrigido checando `matchMedia('(prefers-reduced-motion: reduce)')` e usando `behavior: 'auto'` quando o usuário pediu.
- **Gate 5, achado real correlato**: `<nav>` sem `aria-label` — se a página tiver outro `<nav>` (ex: header), leitor de tela navegando por landmark ouve "navigation" duas vezes sem distinção. Adicionado `aria-label="Table of contents"`.
- Gate 2, achado real, mesma categoria já corrigida dezenas de vezes: `bg-patina/8` (item ativo) reimplementava à mão o que `bg-patina-soft` já faz → substituído.
- Gate 1: aspas simples → duplas nos tipos.
- Showcase: os IDs dos itens não existiam em lugar nenhum do DOM — clicar não fazia nada (`getElementById` retornava `null`) e o scroll nunca achava nenhum alvo, sempre preso no primeiro item. Adicionados âncoras reais (`id={item.id}`) no fluxo normal da página (não um container de scroll isolado — o componente rastreia `window.scrollY`, não um painel contido, então os alvos precisam estar no scroll real da página).
- Gate 9: `e2e/cn/layout/scroll-spy.spec.ts` novo (4 testes: crash/console/`nav` com `aria-label` + `aria-current="location"` certo/clicar rola até a seção mesmo com `prefers-reduced-motion`) — 8/8 chromium-desktop + mobile-chrome, direto na primeira rodada.

### `display/spotlight` — concluído

Centésimo trigésimo terceiro standalone. Diferente dos outros achados desta segunda leva: `Card(5 effects, wrappers genuínos)` já foi confirmado como absorção **verdadeira** na varredura de pendência 0 — `Spotlight` é um wrapper backward-compat real e correto sobre `Card effect="spotlight"` (`display/card` já validado numa sessão bem anterior). Conferido de qualquer forma pelos 9 gates, como todo componente desta fila.

- Wrapper confirmado correto: props (`children`/`color`/`size`/`className`/`style`) e defaults (`color='var(--ks-violet-soft)'`, `size=300`) idênticos aos do `SpotlightImpl` dentro do `Card.tsx` — nenhuma divergência.
- Gate 1, limpeza: aspas simples → duplas nos tipos.
- Registry já vinha 100% correto (`registryDeps: [card]` confirmado automático no `registry:build`).
- Gate 2/5: efeito é puramente decorativo e mouse-only por natureza (glow radial acompanhando o cursor, `pointer-events-none`, sem informação nenhuma pra comunicar) — sem achado de a11y, consistente com o padrão já usado no `Card.tsx` original.
- Gate 9: `e2e/cn/display/spotlight.spec.ts` novo (3 testes: crash/console/mover o mouse revela o glow via `opacity: 1` real) — 5 passed + 1 skip **justificado por plataforma, não pendência 0b**: mobile-chrome emula touch, sem conceito de hover/cursor — o efeito não tem como ser exercitado nesse ambiente por natureza, confirmado que passa em chromium-desktop.

### `display/stepper` — concluído

Centésimo trigésimo quarto standalone. `stepper→dot-stepper/progress-steps` já confirmado como absorção falsa, mas `Stepper` em si nunca tinha passado pelos 9 gates.

- **Gate 5, achado grave real, mesma categoria do `PinBoard`/`Resizable`/`CardStack`**: círculo clicável (`clickable && status==='completed'`) era `<div onClick>` — zero alternativa de teclado. Corrigido convertendo pra `<button>` real quando interativo (Enter/Space de graça, sem precisar de `onKeyDown` manual) via um `StepCircle` compartilhado entre as duas orientações.
- **Gate 5, achado real, mesmo padrão já estabelecido no `StepForm`/`ProgressSteps`**: sem `role="list"`/`"listitem"` nem `aria-current="step"` em nenhuma das duas orientações (horizontal e vertical são dois branches de render totalmente separados, cada um precisou do fix independentemente).
- **Achado real correlato no showcase, achado durante o Gate 9**: `clickable` estava demonstrado mas **sem nenhum efeito real** — a demo passava `activeStep={ctrl.step}` (controlado) sem passar `onStepChange`, então clicar num círculo completo chamava o callback do componente (correto) mas nada escutava — o recurso parecia funcionar visualmente (cursor pointer) mas não fazia nada. Corrigido conectando `onStepChange={ctrl.goTo}`.
- Gate 2, achado real: sufixo "(optional)"/"\*" usava `opacity-60` sem cor própria — herdava a cor do status (patina/danger/etc.) diluída, mudando de cor conforme o step; → `text-faint`, neutro e consistente independente do status.
- Gate 1, limpeza: `import React` nunca usado → removido; aspas simples → duplas.
- Gate 3: `gap-4`/`pb-6`/`mt-4`/`pt-1`/`mt-[2px]`/`mt-3`/`mb-8`/`mt-0.5` (todos match exato, arquivo nunca tinha sido tokenizado) → tokens de spacing; `mt-[0.4rem]` (6.4px, entre `xs` e `sm`) documentado sem match exato, mantido.
- **Achado real no registry**: descrição de `steps` omitia `optional?` (real); `children` (real, existe no componente) não estava documentado. Corrigidos + `registry:build`.
- Showcase: só tinha a variação horizontal — adicionada uma seção vertical com step opcional e step com erro.
- Gate 9: `e2e/cn/display/stepper.spec.ts` novo (5 testes: crash/console/`role="list"`+`aria-current="step"`/círculo clicável navegável por Enter — achado principal, revelou o gap do `onStepChange` na demo/step opcional e com erro usam os tokens certos) — 10/10 chromium-desktop + mobile-chrome. Achado de teste: `getByText("(optional)")` sem `exact` colidia com o próprio painel de código-fonte ao vivo da página (mostra o JSX literal); resolvido com `exact: true`.

### `display/tag` — concluído

Centésimo trigésimo quinto standalone. `badge→tag/status-badge/ping` já confirmado como absorção falsa, mas `Tag` em si nunca tinha passado pelos 9 gates.

- **Gate 5, achado grave real, categoria nova nesta auditoria**: `Root` virava `<button>` nativo quando `onClick` estava presente, e o botão de remover é **sempre** um `<button>` aninhado dentro do `Root` quando `removable`/`onRemove` também estão presentes — `<button>` dentro de `<button>` é HTML inválido (o parser do browser fecha o outer cedo, quebrando clique/foco de forma silenciosa) sempre que `onClick` + `removable` coexistem na mesma tag. Corrigido trocando `Root` de `button`/`span` condicional pra sempre `<span>`, com `role="button"`/`tabIndex={0}`/`onKeyDown` (Enter/Espaço) quando `isClick` — o botão de remover continua sendo um `<button>` real e válido, agora sem aninhamento. Showcase ganhou uma instância `onClick` + `removable` juntos especificamente pra exercitar esse caso, e o Playwright confirma via `tagName === "SPAN"` que o outer não é mais um `<button>`.
- Gate 5, achado real: `aria-label="Remove"` no botão de remover era genérico — múltiplas tags removíveis na mesma tela (caso comum de lista de chips) ficam todas com o mesmo nome acessível "Remove", ambíguo pra leitor de tela. Corrigido pra `` `Remove ${children}` `` quando `children` é string (fallback genérico "Remove" quando não é).
- Gate 2, achado real (categoria recorrente da auditoria): variantes `soft` de `primary`/`info`/`success`/`warning`/`danger` usavam diluição ad-hoc (`bg-patina/10 text-patina`, etc.) em vez do par de tokens `-soft`/`-soft-fg` já existente (`bg-patina-soft text-patina-soft-fg`) — mesmo padrão já usado pelo `Badge` (componente irmão, já validado). Corrigido pra usar os tokens `-soft`/`-soft-fg` nas 5 intents coloridas.
- Gate 2, achado real: `neutral/solid` usava `bg-foreground text-base`, inconsistente com a convenção já estabelecida no `Badge` (`bg-neutral text-neutral-fg`) pro mesmo par intent/variante — corrigido pra bater com o `Badge`. `neutral/soft` (`bg-graphite text-foreground`) já batia com o `Badge` e foi mantido como está (documentado com comentário — é a convenção real da lib pra neutral soft, não o par genérico `bg-neutral-soft`/`text-neutral-soft-fg`).
- **Achado real no registry**: `intent` documentava `'secondary'` como valor válido — não existe no `TagIntent` real (`primary | info | success | warning | danger | neutral`), tipo nunca teve essa variante. Corrigido removendo do enum documentado. `style` (real, existe em `TagProps`) não estava documentado — adicionado. `registry:build` rodado.
- Showcase: só cobria intents/appearances/removable/clickable isolados — adicionadas as 3 variações de `size`, uma instância com `icon`, e a combinação `onClick` + `removable` (achado do Gate 5 acima) com estado real (`removed`/`clicks`) pro Playwright poder verificar efeito de fato, não só presença visual.
- Gate 9: `e2e/cn/display/tag.spec.ts` novo (5 testes: crash/console/clique+teclado no clicável incrementando contador real/achado principal — `tagName` do combo `onClick`+`removable` é `SPAN` não `BUTTON`/label do botão de remover inclui o texto da tag) — 10/10 chromium-desktop + mobile-chrome.

### `inputs/time-picker` — concluído

Centésimo trigésimo sexto standalone — nunca tinha passado pelos 9 gates.

- **Achado real, categoria funcional nova nesta auditoria**: `isEmpty` (controla se mostra o placeholder ou o valor) checava `val.hours === 0 && val.minutes === 0` — selecionar meia-noite (`00:00`) de propósito, no modo uncontrolled sem `defaultValue`, ficava **indistinguível** de "nada selecionado ainda", então o placeholder continuava exibido pra sempre mesmo depois do clique do usuário registrar corretamente (`onChange` disparava certo, só o texto exibido mentia). Corrigido com um estado `touched` que rastreia interação real, independente de qual valor foi escolhido; em modo controlled, `isEmpty` agora é sempre `false` (o consumidor sempre fornece um valor concreto nesse modo, não existe estado "vazio" representável em `TimeValue`).
- **Gate 5, achado real, mesmo padrão já corrigido no `NavigationMenu`**: popover fechava ao clicar fora, mas não tinha `Escape` nenhum — todo outro widget flutuante da lib fecha com Escape, esse era mais um caso do mesmo gap. Corrigido com o mesmo listener `document.addEventListener('keydown', ...)`.
- **Achados reais no registry (padrão quase universal desta auditoria)**: `format` documentava default `'12'`, o real é `'24'`; `minuteStep` documentava default `1`, o real é `5`; `placeholder` documentava default `undefined`, o real é `'Select time'`; `className`/`style` (reais, existem em `TimePickerProps`) não estavam documentados. Descrição também prometia "keyboard input" — não existe campo de texto digitável, é só um dropdown de colunas clicáveis; corrigida pra descrever o componente real. Todos corrigidos + `registry:build`.
- Gate 1, limpeza: `time-picker.types.ts` e `index.ts` usavam aspas simples e sem `;` (único par de arquivos do componente ainda não convertido) → aspas duplas + `;` no padrão do projeto.
- Gate 2/3: sem cor hardcoded, sem tipografia arbitrária fora do já documentado (`text-[0.625rem]` nos três micro-labels de coluna "Hour"/"Min"/"Period", já tinha comentário de exceção `below scale minimum` correto desde antes). `shadow-[...]` com `oklch(...)` literal no popover: sem token equivalente na lib inteira (nenhum arquivo de tokens define `--ks-shadow-*`), mesma situação de todo outro popover/dropdown já auditado — não é bug pontual deste componente.
- Showcase: as duas instâncias existentes eram só `controlled` (portanto nunca exercitavam o bug do `isEmpty`/`touched`) e não cobriam `minuteStep` custom nem `disabled`. Adicionadas duas: uma uncontrolled com `minuteStep={15}` (exercita o fix do achado principal) e uma `disabled`.
- Gate 9: `e2e/cn/inputs/time-picker.spec.ts` novo (5 testes: crash/console/Escape fecha o popover/achado principal — meia-noite uncontrolled atualiza o texto exibido/disabled não abre) — 10/10 chromium-desktop + mobile-chrome.

### `data/tree-view` — concluído

Centésimo trigésimo sétimo standalone, último item da segunda varredura de lacunas de cobertura desta sessão (17/17 fechados). Nunca tinha passado pelos 9 gates.

- **Gate 5, achado grave real, categoria nova nesta auditoria**: `role="tree"` no registry e no componente prometia "keyboard navigation", mas cada `treeitem` era um `<button>` independente navegável só via `Tab` — numa árvore com 7 nós isso são 7 paradas de tab, **zero** tecla de seta funcionando (nenhum suporte ao padrão ARIA de treeview real: roving tabindex + Up/Down/Right/Left/Home/End). Implementado do zero: `activeId` com roving tabindex (só o item ativo tem `tabIndex=0`, os demais `-1`), navegação por `ArrowDown`/`ArrowUp` entre itens visíveis (via lista achatada que respeita `expanded`), `ArrowRight` expande um nó colapsado ou entra no primeiro filho de um já expandido, `ArrowLeft` colapsa um nó expandido ou sobe o foco pro nó pai (mapa de pais construído por travessia), `Home`/`End` pulam pro primeiro/último item visível. Itens `disabled` são excluídos da lista navegável (o próprio atributo `disabled` do `<button>` já os tira do foco nativamente, a lista de navegação só precisa não tentar apontar pra eles). Foco imperativo via `Map<string, HTMLButtonElement>` de refs, mesma técnica já usada em outros pontos da lib pra listas dinâmicas.
- Gate 2, achado real (categoria recorrente): linha selecionada usava diluição ad-hoc `bg-patina/15 text-patina` em vez do par de tokens `-soft`/`-soft-fg` já existente → `bg-patina-soft text-patina-soft-fg`.
- Gate 5, achado pequeno: ícone de nó (`node.icon`) não tinha `aria-hidden="true"` — o chevron de expandir/colapsar ao lado já seguia esse padrão, o ícone do nó (puramente ilustrativo, o `label` já carrega o significado completo) ficou inconsistente. Corrigido pra bater com o chevron.
- **Achado real no registry**: `style` (real, existe em `TreeViewProps`) não estava documentado. Corrigido + `registry:build`.
- Gate 1/3: sem achados — `tree-view.types.ts`/`index.ts` já usavam aspas duplas e `;`; `py-[0.3125rem]` (5px, sem match exato na escala) e o `paddingLeft` inline por profundidade já vinham documentados com comentário de exceção correto desde antes.
- Showcase já cobria bem o caso (árvore de 2 níveis, nó `disabled`, `selected` controlado) — nenhuma mudança necessária, só reaproveitado pro Gate 9.
- Gate 9: `e2e/cn/data/tree-view.spec.ts` novo (7 testes: crash/console/roving tabindex tem exatamente 1 `tabIndex=0`/achado principal — `ArrowDown` move foco, `ArrowLeft` sobe pro pai, `ArrowRight` expande sem perder foco/disabled não navegável) — 14/14 chromium-desktop + mobile-chrome.
...
0. **Achado sistêmico — ✅ VARREDURA COMPLETA**: o campo `absorbs` de `cn-registry.tsx` se provou **falso oito vezes** (`avatar`→`avatar-group`, `accordion`→`accordion-group`/`multi-accordion`/`collapsible`, `grid-pattern`→`particle-field`, `scroll-spy`→`table-of-contents`, `stepper`→`dot-stepper`/`progress-steps`, `badge`→`tag`/`status-badge`/`ping`, e dois casos **parciais**: `tooltip`→`context-card` e `input`→`password-input`/`search-input`, ambos dentro de listas majoritariamente verdadeiras) — em todos os casos os componentes "absorvidos" são implementações paralelas reais (ou, nos parciais, têm funcionalidade real não-redundante), nunca de fato unificadas/replicadas, e ficam escondidos da sidebar por isso (`getVisibleComponents()` filtra qualquer nome em `absorbs`). **Todas as ~20 entradas de `absorbs:` em `cn-registry.tsx` foram conferidas nesta sessão** — não é mais pendência, registrado aqui só como referência dos critérios usados: abrir o arquivo do Super e conferir se há de fato um dispatcher (`switch`/discriminated union) que produz a MESMA funcionalidade do absorvido, como em `Table`/`Progress`/`Timeline`/`Chart`/`TextEffect`/`Card`/`Button`/`Select` (confirmados verdadeiros), e não uma implementação solta ou um equivalente-mas-incompleto, como em `Avatar`/`Accordion`/`GridPattern`/`ScrollSpy`/`ContextCard`/`Stepper`/`Badge`/`PasswordInput`+`SearchInput` (confirmados falsos ou parciais). Lista completa de verdadeiros confirmados: `Table`(data-grid/data-list/tree-table), `Progress`(progress-ring/gauge/skill-bar), `Timeline`(scroll-timeline/timeline-progress/activity-feed), `Chart`(6 renderers, é router não absorção), `Modal`(alert-dialog/drawer/side-panel), `Command`(command-bar/spotlight-search), `DropdownMenu`(context-menu/floating-menu), `ToggleGroup`(segmented-control/chip-group/filter-bar), `Stat`(metric-card/stats-card), `DatePicker`(date-range-picker/calendar), `Kbd`(shortcut-key), `Tooltip`(rich-tooltip/popover/hover-card, SEM context-card), `Card`(5 effects, wrappers genuínos), `Button`(magnetic/confetti/confirm — confirm-button não delegava, corrigido), `Input`(number/currency/phone/floating-label, SEM password/search), `Select`(multi/rich/combobox, dispatch real via `switch(mode)`), `Rating`(rating-input), `Slider`(range-slider). Se um novo Super component for adicionado no futuro com `absorbs`, aplicar o mesmo critério antes de confiar no campo.
   0b. ✅ **FECHADO**: em `mobile-chrome` (~393px) e qualquer viewport abaixo de 860px, a sidebar do showcase (`cn-sidebar.tsx`) não colapsava pra menu hambúrguer e o grid de demos caía pra 2 colunas — a caixa de conteúdo do `Frame` ficava espremida a ~66px de largura total (conteúdo útil ≈ 0px depois do padding). Confirmado idêntico em `grid-pattern`/`particle-field`/`pin-board`/`infinite-scroll` (não relacionados entre si) — afetava **qualquer** demo que dependesse de largura real pra renderizar algo visível (canvas, SVG com `viewBox` proporcional, sentinel de `IntersectionObserver` com `width:0`, etc.) nesse breakpoint. Resolvido colapsando a sidebar pra drawer: `src/app/[locale]/cn/layout.tsx` virou client component com `mobileNavOpen`/`isDrawerMode` (`matchMedia`), `<aside>` fica `position: fixed` + `translateX(-100%)` abaixo de 860px (mesmo breakpoint em que a nav principal do header já sumia) e desliza pra dentro sobre um backdrop quando aberta; `cn-header.tsx` ganhou o botão hambúrguer (`.cnh-menu-btn`, visível só nesse breakpoint). Fecha com clique no backdrop, `Escape`, ou navegação (auto-close via `useEffect` no `pathname`); `inert` real (não `inert=""`, que o React trata como `false` e quebra a hidratação) tira a sidebar do tab order enquanto fechada nesse modo, sem afetar o desktop (`inert` só ativa quando `isDrawerMode && !mobileNavOpen`). Verificado via Playwright: `e2e/cn/cn-sidebar.spec.ts` novo (3 testes: desktop sem hambúrguer/sidebar estática, mobile hambúrguer abre+fecha por Escape+backdrop+`inert` correto, auto-close ao navegar) — 6/6 chromium-desktop + mobile-chrome. Efeito colateral positivo confirmado: o skip de pendência-0b que existia em `context-card.spec.ts` pro teste de hover em `mobile-chrome` foi removido e o teste passa normalmente agora (16/16, zero skips) — o squeeze era mesmo a causa raiz desse tipo de falha em cascata.
   0c. ✅ **REBAIXADO/FECHADO como achado de componente** (revisitado, ver seção `inputs/button (revisita)` acima): `Button.tsx` modo `confirm="doubleclick"` foi CONFIRMADO CORRETO pra usuários reais (reprodução ao vivo via MutationObserver, clique real e programático ambos funcionam e revertem no `resetDelay` esperado). O que resta é uma anomalia confirmada, isolada ao Playwright especificamente na rota `feedback/confirm-button` (mesma demo funciona via Playwright em `inputs/button`) — ads/reCAPTCHA, foco/visibilidade e hydration incompleta descartados como causa. Causa raiz do porquê-só-nessa-rota não isolada; documentada como curiosidade de test-infra em `confirm-button.spec.ts` (`test.fail` com comentário atualizado), não bloqueia nada
1. ✅ **FECHADO**: `ContextCard` aposentado em favor de `<Tooltip variant="card">` (decisão confirmada com o usuário). `ContextCard.tsx` reescrito como wrapper genuíno (mesmo padrão de `RichTooltip`/`Popover`/`HoverCard`), `absorbs` do `tooltip` no registry corrigido pra incluir `context-card` (era `false` por engano). `delay` (documentado como não implementado na versão CSS-only) agora funciona de verdade via `openDelay`/`closeDelay` do Tooltip. `e2e/cn/overlays/context-card.spec.ts` reescrito pro novo DOM (portal + `role="tooltip"`, não mais `.cc-popup`) — 5 testes, 10/10 chromium-desktop + mobile-chrome
...
3. ~~`cn-install-block/CnInstallBlock.tsx` usa hex cru~~ — ✅ FECHADO. Confirmado exceção válida (bloco de terminal/CLI, paleta GitHub dark deliberadamente fixa, independente do tema claro/escuro do site). Comentário de exceção adicionado. De passagem, achado igual em `terminal-block/TerminalBlock.tsx` (mesmo `bg-[#0d1117]` sem comentário) — também documentado; e um `text-sm` cru (banido) no mesmo arquivo → `text-body-callout` (match exato)
   4b. ✅ **FECHADO**: `QuickActions` e `Fab` implementavam o mesmo conceito (FAB circular que expande num speed-dial de ações) de forma totalmente independente, sem `absorbs` ligando um ao outro (decisão confirmada com o usuário: marcar absorção real, `Fab` como Super). `Fab` ganhou 3 features exclusivas do `QuickActions` — `position="inline"` (renderiza no fluxo normal em vez de `fixed` num canto), `placement` de 4 direções (antes só inferia cima/baixo a partir do canto fixo) e `intent` por ação individual — mais um bônus de acessibilidade de passagem (`group-focus-within` no rótulo da ação, já corrigido no `QuickActions` original mas faltando no `Fab`). `QuickActions.tsx` virou wrapper genuíno (`<Fab position="inline" tooltip="Quick actions" .../>`, o `tooltip` fixo preserva o nome acessível original — sem ele o fallback do Fab produziria "Abrir menu"/"Fechar"). **Duas diferenças de comportamento aceitas conscientemente na fusão**: (1) as ações agora empurram o layout ao abrir (flex, padrão do Fab) em vez de sobrepor por cima via `position: absolute` como o `QuickActions` original fazia; (2) ação escondida (menu fechado) agora desmonta de vez (`{hasActions && open && ...}`, padrão que o `Fab` já usava) em vez de ficar montada com `tabIndex={-1}` como o `QuickActions` original fazia — ambos padrões de acessibilidade válidos, só motivos diferentes de implementação. `registry:build` roda `registryDeps: [fab]` pro `quick-actions`, confirmando a delegação real. `e2e/cn/inputs/fab.spec.ts` +1 teste (`inline`/`placement`×4/`intent` por ação); `e2e/cn/overlays/quick-actions.spec.ts` reescrito pras novas expectativas (nome acessível preservado, ação desmontada quando fechada) — 23/26 chromium-desktop + mobile-chrome (3 skips são a pendência 0b em specs de speed-dial já documentada, pré-existente, não nova)
4. ✅ **FECHADO**: nenhuma `@keyframes` do token bridge (`kikitocn-tokens.css`) respeitava `prefers-reduced-motion: reduce` — achado original via `AnimatedList`, mas a investigação pra fechar mostrou que o problema era maior: a maioria dos popovers/dropdowns (`DatePicker`, `Select` ×2, `TimePicker`, `ImageViewer`) nem usa as keyframes `--animate-*` do token bridge — usa as classes `animate-in`/`fade-in`/`slide-in-from-*` do plugin `tailwindcss-animate` (biblioteca externa, keyframes próprias), e `Progress.tsx` usa `animate-[ks-progress-slide_...]`/`animate-[ks-progress-stripe_...]` arbitrários com keyframes que nem batem com as do token bridge. Caçar cada classe/keyframe individualmente seria frágil (qualquer `animate-[...]` arbitrário novo escaparia). Resolvido com um reset global no fim de `kikitocn-tokens.css` (`@media (prefers-reduced-motion: reduce) { *, *::before, *::after { animation-duration: 0.01ms !important; animation-iteration-count: 1 !important; transition-duration: 0.01ms !important; scroll-behavior: auto !important; } }`) — abordagem padrão (ex.: html5-boilerplate), neutraliza qualquer mecanismo de animação/transição, atual ou futuro, sem exigir que cada componente novo lembre de checar a media query manualmente. `kikitocn-tokens.css` já é importado em `globals.css` pro app inteiro (não só `cn/`), o que é o comportamento correto aqui — preferência de acessibilidade do usuário deve valer pro dashboard também, isso não é uma questão de vocabulário de tokens (que continua separado normalmente). Não conflita com o handling específico já existente do `MarqueeText` (`.mq-track { animation: none }` sob a mesma media query, mais específico, continua vencendo) — na verdade o reset global é a rede de segurança pra todo o resto que não tem tratamento bespoke. Verificado via Playwright (`page.emulateMedia({reducedMotion:'reduce'})`): `animation-duration` computado vira `1e-05s` no `<body>` inteiro sob a preferência, e a animação do `MarqueeText` continua tocando normalmente (10s) sem a preferência ativada — nenhuma regressão visual em movimento normal
...
6. Fonte de verdade das regras de token (cor/tipografia/radius/spacing): **CLAUDE.md na raiz do projeto** — já atualizado, único lugar que precisa ficar em sincronia com o skill `/validate-component`.
   7b. `?? ` misturado com `||` sem parênteses (`a ?? b || c`) é erro de sintaxe real (regra do ECMAScript), mas nem `tsc --noEmit` nem `eslint` acusaram nesta sessão — só o compilador de verdade do dev server (SWC/webpack) rejeitou, derrubando a rota inteira com 500 (achado no `SignaturePad`, Playwright que revelou via `page.goto` travando/erro 500, não lint/typecheck). Sempre que introduzir `??` misturado com `||`/`&&` na mesma expressão, parenteses explícitos são obrigatórios — e `tsc`/`eslint` não são suficientes como única rede de segurança pra isso, só rodar a página de verdade confirma. **Varrido em toda `cn/**`numa sessão futura**: grep de`??`co-ocorrendo com`||`/`&&`na mesma linha achou 9 arquivos, todos inspecionados manualmente — nenhum tinha a mistura ilegal de fato (todos já com o`??`isolado em parênteses próprios antes de qualquer`||`/`&&`, incluindo o `SignaturePad.tsx`já corrigido). Confirmado indiretamente também:`npm run build` completo (produção, SWC de verdade) passou limpo na mesma sessão — se existisse a mistura ilegal em qualquer arquivo do bundle, o build teria falhado no parse, não só um lint silencioso.
7. `eslint-disable-next-line` num atributo JSX que fica em linha própria (padrão do Prettier pra tags com múltiplos atributos) não sobrevive a uma reformatação futura — o Prettier pode reindentar e a diretiva passa a apontar pra linha errada. Pra atributo único (ex: `tabIndex={0}`), usar `// eslint-disable-line <regra>` **na mesma linha do atributo** — sobrevive a `prettier --write` porque o comentário fica preso à linha, não à posição. **Varrido em toda `cn/**`numa sessão futura**: achados reais em`Command.tsx`/`Select.tsx`/`Table.tsx` (`autoFocus`numa linha própria com`// eslint-disable-next-line jsx-a11y/no-autofocus`acima) e`MarkdownRenderer.tsx` (`dangerouslySetInnerHTML`com`react/no-danger`acima) — os 4 corrigidos pro padrão inline. Os demais`eslint-disable-next-line` do repo ficam antes de uma tag JSX inteira (`<div ...`) ou de uma linha de fechamento de hook (`}, [deps]);`), não de um atributo único — posição já estável, não precisam de mudança.
