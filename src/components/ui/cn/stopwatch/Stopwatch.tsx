...
export function Stopwatch({ initialTime = 0, onLap, showLaps = true, maxLaps = 20, className, style }: StopwatchProps) {
    ...
    <div className={cn("flex flex-col items-center gap-(--spacing-lg)", className)} style={style}>
      ...
      <div className="flex gap-(--spacing-sm)">
        {!running ? (
          ...
          <Button intent="warning" variant="solid" size="md" onClick={pause}>
            Pause
          </Button>
        )}
        {running && (
          <Button intent="neutral" variant="outline" size="md" onClick={lap}>
            Lap
          </Button>
        )}
        <Button
