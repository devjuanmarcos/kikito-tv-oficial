...
const VARIANT_CLS: Record<TextareaVariant, string> = {
  outline: 'bg-raised border border-rule hover:border-foreground/40 focus:border-patina focus:shadow-[0_0_0_3px_color-mix(in_oklch,var(--ks-patina)_18%,transparent)]',
  filled:  'bg-graphite border border-transparent hover:bg-graphite-2 focus:bg-graphite-2 focus:border-patina focus:shadow-[0_0_0_3px_color-mix(in_oklch,var(--ks-patina)_18%,transparent)]',
  ghost:   'bg-transparent border-0 border-b border-rule rounded-none hover:border-foreground/40 focus:border-b-patina outline-none',
}
...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(function Textarea({
  ...
  const charCount = typeof value === 'string' ? value.length
  ...
  useEffect(() => { adjustHeight() }, [value, defaultValue])
  ...
  const feedbackText =
    ...
    effectiveState === 'warning' ? warningText :
    helperText
  ...
  return (
    <div className={cn('flex flex-col gap-[0.375rem]', className)}>
      ...
      <div className="flex items-center justify-between gap-2">
        {feedbackText && (
          <span className={cn('text-body-caption', STATE_TEXT_CLS[effectiveState])}>
            {feedbackText}
          </span>
        )}
        {showCount && (
          <span className={cn('text-body-caption ml-auto tabular-nums', maxLength && charCount >= maxLength * 0.9 ? 'text-warning' : 'text-faint')}>
