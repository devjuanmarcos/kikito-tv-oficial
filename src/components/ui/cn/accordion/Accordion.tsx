...
const ChevronDown = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-[0.9rem] h-[0.9rem]">
      ...
      {items.map(item => {
        ...
        return (
          <div key={item.value} className={cn(VARIANT_ITEM_EXTRA[variant])}>
            ...
            >
              <div className={cn(variant === 'ghost' ? 'py-2 text-body-callout text-muted' : 'px-4 pb-4 text-body-callout text-muted')}>
                {item.children}
              </div>
            </div>
          </div>
