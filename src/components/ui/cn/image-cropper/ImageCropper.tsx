...
export function ImageCropper({
  ...
  const dragRef = useRef<{ type: 'move' | 'tl' | 'tr' | 'bl' | 'br'; startX: number; startY: number; startCrop: CropArea } | null>(null)

  const clamp = (v: number, min: number, max: number) => Math.max(min, Math.min(max, v))

  const onMouseDown = useCallback((e: React.MouseEvent, type: 'move' | 'tl' | 'tr' | 'bl' | 'br') => {
    e.stopPropagation()
    e.preventDefault()
    ...
    const onMove = (me: MouseEvent) => {
      ...
      if (dragRef.current.type === 'move') {
        ...
        next.y = sc.y + sc.height - nh; next.height = nh
      } else if (dragRef.current.type === 'bl') {
        const nw = clamp(sc.width - dx, 10, sc.x + sc.width)
        next.x = sc.x + sc.width - nw; next.width = nw
  ...
  return (
    <div
      ref={containerRef}
      className={cn('relative overflow-hidden bg-canvas rounded-[--radius-md] border border-rule cursor-crosshair select-none', className)}
      style={style}
    ...
    >
      ...
      >
        <div className="absolute w-[10px] h-[10px] bg-patina rounded-[2px] -top-[5px] -left-[5px] cursor-nwse-resize" onMouseDown={e => onMouseDown(e, 'tl')} />
        <div className="absolute w-[10px] h-[10px] bg-patina rounded-[2px] -top-[5px] -right-[5px] cursor-nesw-resize" onMouseDown={e => onMouseDown(e, 'tr')} />
        <div className="absolute w-[10px] h-[10px] bg-patina rounded-[2px] -bottom-[5px] -left-[5px] cursor-nesw-resize" onMouseDown={e => onMouseDown(e, 'bl')} />
