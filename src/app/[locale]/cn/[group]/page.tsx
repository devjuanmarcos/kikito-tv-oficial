...
export default async function CnGroupPage({ params }: Props) {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-8">
        <Link href="/cn" className="text-[0.8125rem] text-muted hover:text-foreground inline-flex items-center gap-1 mb-4 transition-colors">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" width={14} height={14}><path d="M10 12L6 8l4-4"/></svg>
          Todos os grupos
        </Link>
        <div className="flex items-center gap-3 mb-2">
          <span className="text-[1.5rem]">{groupMeta.icon}</span>
          <h1 className="text-[1.75rem] font-bold text-foreground">{groupMeta.label}</h1>
        </div>
      ...
      <div className="grid gap-3">
        {components.map(comp => (
          ...
          >
            <div>
              <h2 className="text-[0.9375rem] font-semibold text-foreground group-hover:text-patina transition-colors">{comp.title}</h2>
              <p className="text-[0.8125rem] text-muted mt-[2px]">{comp.description}</p>
            </div>
