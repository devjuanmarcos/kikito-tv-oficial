...
const AnimatedCanopy = ({
  ...
  <div
    {...props}
    className={cn(
      "group relative flex h-full w-full overflow-hidden p-2 [--duration:10s] [--gap:12px] [gap:var(--gap)] h-fit",
  ...
  >
    <div className="flex items-start gap-3">
      <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full border-2 border-border">
        <img src={testimonial.image} alt={testimonial.name} className="h-full w-full not-prose object-cover" />
      </div>
      <div className="flex-1">
        <div className="flex items-baseline gap-2">
          <span className="body-paragraph-bold ">{testimonial.name}</span>
          <span className="body-caption text-muted-foreground">{testimonial.handle}</span>
        </div>
        <p className="mt-1 line-clamp-3 body-callout ">{testimonial.description}</p>
      </div>
    </div>
  </div>
);
    ...
    {[false, true, false].map((reverse, index) => (
      ...
      >
        {data.map((testimonial) => (
          <TestimonialCard key={testimonial.name} testimonial={testimonial} className={cardClassName} />
        ))}
      </AnimatedCanopy>
    ))}
  </div>
