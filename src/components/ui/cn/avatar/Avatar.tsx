...
export function Avatar({
  ...
  return (
    <span className={cn('relative inline-flex shrink-0', SIZE_DIM[size], VARIANT_CLS[variant], className)} style={style}>
      {showImg ? (
        <img
          src={src}
          alt={alt ?? name ?? ''}
        ...
        <span
          className={cn('w-full h-full flex items-center justify-center font-semibold select-none', VARIANT_CLS[variant])}
        ...
        <span className={cn('w-full h-full bg-graphite-2 flex items-center justify-center text-faint', VARIANT_CLS[variant])}>
