    ...
    <>
      ...
      {open && createPortal(
        ...
        >
          {hasHeader && (
            <div className={cn('flex items-start gap-2 px-4 pt-4', content || footer ? 'pb-2' : 'pb-4')}>
              <div className="flex-1 min-w-0">
                {title && (
                  <p className="text-body-callout font-semibold text-foreground leading-tight">{title}</p>
                )}
                {description && (
                  <p className="text-body-caption text-faint mt-0.5 leading-[1.5]">{description}</p>
                )}
              </div>
              {showClose && (
                <button
          ...
          {content && (
            <div className={cn('px-4', hasHeader ? 'pt-0 pb-3' : 'pt-4 pb-3', !footer && 'pb-4')}>
              {content}
            </div>
          )}
