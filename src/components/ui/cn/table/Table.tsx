...
const SortNone  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[14px] h-[14px]"><path d="M7 15l5 5 5-5"/><path d="M7 9l5-5 5 5"/></svg>
const SortAsc   = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[14px] h-[14px]"><path d="M12 4v16m-5-5l5 5 5-5"/></svg>
const SortDesc  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[14px] h-[14px]"><path d="M12 20V4M7 9l5-5 5 5"/></svg>
const FilterIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3 flex-shrink-0"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>
const PlusIcon   = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3 flex-shrink-0"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
const XIcon      = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
const CheckIcon  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-[10px] h-[10px]"><polyline points="20 6 9 17 4 12"/></svg>
const ViewIcon   = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[14px] h-[14px]"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>
const SearchIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3 text-faint flex-shrink-0"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
const ChevL  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[14px] h-[14px]"><polyline points="15 18 9 12 15 6"/></svg>
const ChevR  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[14px] h-[14px]"><polyline points="9 18 15 12 9 6"/></svg>
const ChevsL = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[14px] h-[14px]"><polyline points="11 17 6 12 11 7"/><polyline points="18 17 13 12 18 7"/></svg>
const ChevsR = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-[14px] h-[14px]"><polyline points="13 17 18 12 13 7"/><polyline points="6 17 11 12 6 7"/></svg>
const EmptyIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-8 h-8"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>

/* ── Primitive table elements ── */

export function Table({ children, striped, size, className, style }: {
  children?: ReactNode
  striped?:  boolean
  size?:     'sm' | 'md' | 'lg'
  className?: string
...
export function TableHead({ children, sticky, className }: { children?: ReactNode; sticky?: boolean; className?: string }) {
  return (
...
export function TableRow({ children, selected, onClick, className }: {
  children?: ReactNode
  selected?: boolean
  onClick?:  () => void
  className?: string
...
export function TableCell({ children, align, pinRight, colSpan, className, style }: {
  children?: ReactNode
...
export function TableHeadCell({ children, align, sortable, sorted, onSort, pinRight, colSpan, className, style }: {
  children?: ReactNode
  align?:    'left' | 'center' | 'right'
  sortable?: boolean
  sorted?:   SortDir | null
  ...
  return (
    ...
    >
      <span className="inline-flex items-center gap-[5px]">
        {children}
        {sortable && (
          <span className={cn('inline-flex items-center opacity-40 transition-opacity flex-shrink-0', sorted && 'opacity-100 text-patina')}>
...
function SelectFilter({ title, options, value, multiple, onChange }: {
  title: string
...
function ViewOptions({ columns, hidden, onToggle }: {
...
function PaginationBar({ page, totalPages, pageSize, totalItems, pageSizeOptions, onChange, onPageSizeChange }: {
  page: number; totalPages: number; pageSize: number; totalItems: number
  pageSizeOptions: number[]; onChange: (p: number) => void; onPageSizeChange: (ps: number) => void
}) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 flex-wrap">
      ...
      <div className="flex items-center gap-1">
        <button className={pageBtnCls} onClick={() => onChange(0)} disabled={page === 0} aria-label="First page"><ChevsL /></button>
              ...
              {visibleCols.map(col => (
                ...
                >
                  <span className="inline-flex items-center gap-[5px]">
                    {col.header}
                    {col.sortable && (
                      <span className={cn('inline-flex items-center opacity-40 transition-opacity flex-shrink-0', sortKey === col.key && 'opacity-100 text-patina')}>
                        {sortKey === col.key && sortDir === 'asc' ? <SortAsc /> : sortKey === col.key ? <SortDesc /> : <SortNone />}
                      </span>
                    )}
                  </span>
                </th>
            ...
            {loading ? (
                ...
                <tr key={ri} className="border-b border-rule animate-pulse">
                  ...
                  {visibleCols.map(col => (
                    <td key={col.key} className={tdCls}>
                      <div className="h-4 bg-graphite rounded-[3px]" style={{ width: `${55 + (ri * 13 + col.key.length * 7) % 35}%` }} />
                    </td>
                  ))}
                </tr>
              ))
              ...
              paged.map((row, ri) => {
                ...
                return (
                  ...
                  >
                    {selectable && (
                      <td className={cn(tdCls, 'w-10 pr-1')} onClick={e => { e.stopPropagation(); toggleRow(id) }}>
                        <input
                          type="checkbox"
                          className="w-4 h-4 border border-rule rounded-[3px] bg-transparent cursor-pointer accent-patina"
                          checked={isSelected}
