...
export function Sparkline({
  ...
  function x(i: number) { return pad + (i / (data.length - 1)) * (width - pad * 2) }
  function y(v: number) { return pad + ((max - v) / range) * (height - pad * 2) }
  ...
  return (
    <svg width={width} height={height} className={className} style={style}>
      {(type === 'area' || filled) && (
        <path d={areaPath} fill={resolvedColor} opacity={0.15} />
      )}
      <polyline
        points={points}
        fill="none"
