...
const LABELS: Record<string, string> = {
  online: 'Online', offline: 'Offline', busy: 'Busy', away: 'Away', idle: 'Idle',
}
...
const SIZE_DOT: Record<string, string> = {
  sm: 'w-1.5 h-1.5', md: 'w-2 h-2', lg: 'w-3 h-3',
}
...
export function StatusBadge({
  ...
  return (
    <span className={cn('inline-flex items-center gap-1.5', className)} style={style}>
      ...
      {showLabel && (
        <span className={cn('font-medium text-muted capitalize', SIZE_LABEL[size])}>
          {LABELS[status]}
        </span>
      )}
    </span>
