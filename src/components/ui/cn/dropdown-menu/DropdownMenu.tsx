...
function getMenuPos(trigger: DOMRect, menu: DOMRect, placement: MenuPlacement) {
  ...
  switch (placement) {
    ...
    case 'bottom':       top = tB + GAP; left = tL + tW/2 - mW/2; break
    case 'bottom-end':   top = tB + GAP; left = tR - mW;       break
    case 'top-start':    top = tT - mH - GAP; left = tL;       break
    case 'top':          top = tT - mH - GAP; left = tL + tW/2 - mW/2; break
    case 'top-end':      top = tT - mH - GAP; left = tR - mW;  break
  }
...
export function DropdownMenu({ items, placement = 'bottom-start', children }: DropdownMenuProps) {
  ...
  function renderItem(entry: MenuEntry, i: number): React.ReactNode {
    ...
    return (
      ...
      >
        {entry.icon && (
          <span className="shrink-0 w-3.5 h-3.5 flex items-center justify-center text-faint">
            {entry.icon}
          </span>
        )}
