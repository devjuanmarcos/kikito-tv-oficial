...
export function TextEditor({
  ...
  return (
    <div
      style={style}
      className={cn("flex flex-col rounded-xl border border-rule overflow-hidden bg-raised", className)}
    >
      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-0.5 px-2 py-1.5 border-b border-rule bg-graphite-2">
        {TOOLBAR.map((btn) => (
          <button
            ...
            className={cn(
              "px-2 py-1 rounded-(--radius-sm) text-body-caption font-medium text-faint hover:text-foreground hover:bg-graphite transition-colors",
