...
export function ParticleField({
  count = 60,
  // cor exigida via API do browser (canvas 2D fillStyle/strokeStyle não aceita CSS var) —
  // formato "r,g,b" pra compor rgba() com alpha por partícula/linha. Sem token equivalente.
  color = "120,80,255",
  speed = 0.5,
  size = 2,
  width = "100%",
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const p of particles) {
        ...
        // canvas 2D fillStyle: API do browser não aceita CSS var, precisa de string rgba() literal
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 100) {
            ...
            // canvas 2D strokeStyle: mesma limitação — sem suporte a CSS var
