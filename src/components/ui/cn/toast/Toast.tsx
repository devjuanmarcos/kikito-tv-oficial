...
const InfoIcon    = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
const SuccessIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
...
const DangerIcon  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>
const NeutralIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><circle cx="12" cy="12" r="10"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
const XIcon       = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>

const ICONS = { info: <InfoIcon />, success: <SuccessIcon />, warning: <WarningIcon />, danger: <DangerIcon />, neutral: <NeutralIcon /> }
...
function ToastCard({ item, onDismiss }: { item: ToastItem; onDismiss: (id: string) => void }) {
  ...
  return (
    <>
      ...
      >
        {!isSolid && (
          <span style={{ position: 'absolute', left: 0, top: 8, bottom: 8, width: 3, borderRadius: 999, background: colors.bar }} />
        )}

        {resolvedIcon && (
          <span className="inline-flex items-center shrink-0 pt-[1px]" style={{ color: isSolid ? 'rgba(255,255,255,0.9)' : colors.bar }}>
            {resolvedIcon}
          </span>
        )}
...
export function ToastProvider({ children, placement = 'bottom-right', maxToasts = 5 }: ToastProviderProps) {
  ...
  const toast = useCallback((opts: ToastOptions | string): string => {
    ...
    const item: ToastItem = {
      id, intent, variant, duration, dismissible,
      title:   options.title   ?? '',
