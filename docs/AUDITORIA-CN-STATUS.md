...
1. Bare `rounded` sem sufixo — **grep reconfirmado em 2026-08-27** (Table, Tooltip, Command e DropdownMenu já corrigidos, removidos da lista): `alert/Alert.tsx`, `avatar/Avatar.tsx`, `banner/Banner.tsx`, `checkbox/Checkbox.tsx` (⚠️ verificar antes de mexer — pode ser falso-positivo, o componente tem uma variante literalmente chamada `"rounded"` como string de tipo, não necessariamente uma classe CSS bare), `cn-install-block/CnInstallBlock.tsx`, `code-block/CodeBlock.tsx`, `credit-card/CreditCard.tsx`, `date-picker/DatePicker.tsx`, `event-calendar/EventCalendar.tsx`, `fab/Fab.tsx`, `floating-bar/FloatingBar.tsx`, `image-compare/ImageCompare.tsx`, `inline-edit/InlineEdit.tsx`, `log-viewer/LogViewer.tsx`, `notice-bar/NoticeBar.tsx`, `notification-bell/NotificationBell.tsx`, `select/Select.tsx` (já aprovado nos 9 gates — ficou pra trás, reconfirmar linha exata), `skeleton/Skeleton.tsx`, `stat/Stat.tsx`, `swipe-card/SwipeCard.tsx`, `toast/Toast.tsx`, `video-card/VideoCard.tsx`. 21 arquivos, cada um precisa da mesma verificação manual (é classe CSS bare de verdade, ou falso-positivo tipo variant name?) antes de trocar pelo token certo — não é um sed cego.
2. Bug de sintaxe `-[--var]` (bracket cru) — confirmado quebrado em Tabs/PriceTable/MarkdownRenderer/ContextCard(fixado)/DropdownMenu(nenhum achado aqui); ainda não confirmado se afeta as formas simples sem direção (ver lista completa na seção de achados acima)
3. Decidir o destino de `ContextCard` (aposentar em favor de `<Tooltip variant="card">`, ou investir na reescrita pra JS) — levantado durante o Gate 5 acima, não decidido
4. Cobertura de `focus-visible:outline-patina` em componentes com foco customizado por `role` (Tabs e possivelmente outros `role="tab"`/`role="menuitem"`) que nunca tiveram anel de foco próprio, dependendo só do fallback verde do dashboard — achado colateral do fix do outline global, não é regressão, é lacuna pré-existente
