...
export function EmptyState({ icon, title, description, action, size = "md", className, style }: EmptyStateProps) {
  const s = SIZE_CLS[size];
  return (
    <div className={cn("flex flex-col items-center justify-center text-center", s.pad, s.gap, className)} style={style}>
      {icon && (
        <div
          className={cn("flex items-center justify-center text-muted opacity-50 [&>svg]:w-full [&>svg]:h-full", s.icon)}
        >
          {icon}
        </div>
      )}
      <p className={cn("font-semibold text-foreground m-0", s.title)}>{title}</p>
      {description && <p className={cn("text-muted m-0 max-w-[36ch] leading-normal", s.desc)}>{description}</p>}
      {action && <div className="mt-(--spacing-2xs)">{action}</div>}
    </div>
  );
}
