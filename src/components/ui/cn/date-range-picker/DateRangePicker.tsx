...
function MonthGrid({ year, month, range, hovered, onDay, onHover }: {
  year: number; month: number; range: DateRange; hovered: Date | null;
    ...
    <>
      <div className="grid grid-cols-7 gap-[2px]">
        ...
        {cells.map((day, i) => {
          ...
          const inRange   = !!range.start && !!endDisplay && day > (range.start < endDisplay ? range.start : endDisplay) && day < (range.start < endDisplay ? endDisplay : range.start)
...
export function DateRangePicker({
  ...
  function prev() { if (leftMonth === 0) { setLeftYear(y => y-1); setLeftMonth(11) } else setLeftMonth(m => m-1) }
  function next() { if (leftMonth === 11) { setLeftYear(y => y+1); setLeftMonth(0) } else setLeftMonth(m => m+1) }
  ...
  useEffect(() => {
    if (!open) return
    function h(e: MouseEvent) { if (rootRef.current && !rootRef.current.contains(e.target as Node)) setOpen(false) }
      ...
      {open && (
        <div className="absolute top-[calc(100%+8px)] left-0 z-[200] bg-float border border-rule rounded-[--radius-lg] shadow-[0_8px_32px_color-mix(in_srgb,black_20%,transparent)] p-4 flex gap-4 animate-in fade-in slide-in-from-top-1 duration-[120ms]">
          ...
          <div className="min-w-[220px]">
            ...
            <MonthGrid year={leftYear} month={leftMonth} range={range} hovered={hovered} onDay={handleDay} onHover={setHovered} />
          </div>
          {/* Right month */}
          <div className="min-w-[220px]">
            <div className="flex items-center justify-between mb-3">
            ...
            <MonthGrid year={rightYear} month={rightMonth} range={range} hovered={hovered} onDay={handleDay} onHover={setHovered} />
          </div>
        </div>
      )}
    </div>
